...
class SnakeBoard extends GameBoard
{
    ...
    draw()
    {
        ...
        if(this.appleAmount > apple_list.length)
        {
            while (true)
            {
                var x_pos = randInt(20+this.x,Math.floor((this.x+this.w)/20)*20-20,20)
                var y_pos = randInt(20+this.y,Math.floor((this.y+this.h)/20)*20-20,20)
                var good_to_go = true
                if(this.current_snake == null)
                    break
                for(var i in this.current_snake.tail_array)
                {
                    if((x_pos-this.x)/20 == this.current_snake.tail_array[i][0] && (y_pos-this.y)/20 == this.current_snake.tail_array[i][1])
                    {
                        good_to_go = false
                        break
                    }
