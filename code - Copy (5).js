...
class Snake
{
    ...
    collide()
    {
        if (this.x <= 2 || this.x - this.board.x - this.board.w > -18 || this.y <= 2 || this.y - this.board.y - this.board.h > -18)
        {
            this.lost = true
            this.moving = false
        }
...
class SnakeBoard extends GameBoard
{
    ...
    draw()
    {
        this.y = (Math.cos(frameCount/10)*10)
        if(!this.running)
            return
        super.draw_pre()
        const size = 40
...
function frame()
{
    ...
    {
        boardList[i].allowed_to_run = true
    }
    frameCount++
    if(FPS_Show)
    {
